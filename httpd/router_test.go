...
func TestStaticPaths(t *testing.T) {
	...
	url := fmt.Sprintf("http://%s%s", ws.listener.Addr(), "/")
	rsp, err := c.Get(url)
	assert.Nil(t, err)
	assert.NotNil(t, rsp)
	...
	url = fmt.Sprintf("http://%s%s", ws.listener.Addr(), "/humans.txt")
	rsp, err = c.Get(url)
	assert.Nil(t, err)
	assert.NotNil(t, rsp)
	assert.Equal(t, 200, rsp.StatusCode)
	assert.Equal(t, "text/plain; charset=utf-8", rsp.Header.Get("Content-Type"))
	assert.Equal(t, "13", rsp.Header.Get("Content-Length"))

	url = fmt.Sprintf("http://%s%s", ws.listener.Addr(), "/hello/world.txt")
	rsp, err = c.Get(url)
	assert.Nil(t, err)
	assert.NotNil(t, rsp)
	assert.Equal(t, 200, rsp.StatusCode)
	assert.Equal(t, "text/plain; charset=utf-8", rsp.Header.Get("Content-Type"))
	assert.Equal(t, "17", rsp.Header.Get("Content-Length"))
...
func TestStaticRedirects(t *testing.T) {
	...
	// url = fmt.Sprintf("http://%s%s", ws.listener.Addr(), "/index.html")
	// rsp, err = c.Get(url)
	// assert.NotNil(t, err)
	// assert.NotNil(t, rsp)
	// assert.Equal(t, 301, rsp.StatusCode)
...
func TestAPIPath(t *testing.T) {
	...
	url := fmt.Sprintf("http://%s%s", ws.listener.Addr(), "/api")
	rsp, err := c.Get(url)
	assert.Nil(t, err)
	assert.NotNil(t, rsp)
	assert.Equal(t, 200, rsp.StatusCode)
